...
function App() {
  ...
  return <>
    ...
    />
    <main>
      <section id="inicio" className="hero"><div className="hero-glow" aria-hidden="true" /><div className="container"><div className="hero-content"><p className="eyebrow">{t('hero.eyebrow')}</p><h1 className="hero-title">{t('hero.titlePrefix')} <em>{t('hero.titleAccent')}</em>{t('hero.titleSuffix')}</h1><p className="hero-subtitle">{t('hero.subtitle')}</p><a href="#contactos" className="btn btn-outline">{t('hero.cta')}</a></div><HeroCarousel items={heroCards.map(([image, altKey, titleKey]) => ({ key: image, image: img(image), alt: t(altKey), title: t(titleKey) }))} /></div></section>
      <section id="empresa" className="welcome-section"><div className="container"><div className="welcome-content"><div className="welcome-text"><p className="eyebrow">{t('nav.empresa')}</p><h2>{t('empresa.title')}</h2><p className="section-intro">{t('empresa.text')}</p></div><div className="welcome-image"><img src={img('loja-rnb.jpeg')} alt={t('empresa.imageAlt')} /></div></div><div className="about-grid">{aboutItems.map((key) => <div className="about-item" key={key}><h4>{t(`${key}.title`)}</h4><p>{t(`${key}.text`)}</p></div>)}</div></div></section>
      <section id="servicos" className="services-section"><div className="container"><p className="eyebrow">{t('nav.servicos')}</p><h2>{t('servicos.title')}</h2><p className="section-intro">{t('servicos.intro')}</p><div className="services-grid">{services.map((service) => <article className="service-card" key={service.titleKey}><div className="service-card-image"><img src={img(service.image)} alt={t(service.altKey)} /><span className="service-card-icon"><Icon>{service.icon}</Icon></span></div><div className="service-card-body"><h3>{t(service.titleKey)}</h3><p>{t(service.descKey)}</p></div></article>)}</div></div></section>
      <section id="portfolio" className="portfolio-section"><div className="container"><p className="eyebrow">{t('nav.portfolio')}</p><h2>{t('portfolio.title')}</h2><p className="section-intro">{t('portfolio.intro')}</p><div className="partners-grid">{partners.map(([image, alt]) => <div className="partner" key={image}><img src={img(`partners/${image}`)} alt={alt} /><span className="partner-badge">{t('portfolio.badge')}</span></div>)}</div><p className="portfolio-trust">{t('portfolio.trust')}</p></div></section>
